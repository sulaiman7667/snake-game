				...
				while True:
					...
					if (str(option) == str(1)):
						...
						if(str(suggestionopt) == str(1)):
							updatedsentence = updatedsentence.replace(str(errorslist[m]),str(suggestion[0]))
	...
	if (str(selectedchoice) == str(2)):
		while True:	
			...
			if (os.path.exists(file)):	
				...
				if(str(file).casefold() == "quit"):
					quit()
				print("File not found or path is incorrect. (Type quit to exit program)\n")	
				continue
		t0 = datetime.now()
		time = datetime.now()
		newfile = textfile
...
while True:
	restart = str(input("Select an option: " + "\n1.Restart program." + "\n2.Quit program. \n"))
	if(str(restart) == str(1)):
		main()
		break
	if(str(restart) == str(2)):	
		quit()
		break
	print('You entered an invalid input, please try again.')	
