...
# NOTE: NO STYLING WAS USED DUE TO A BUG IN TKINTER NOT ALLOWING SOME MAC
# USERS TO STYLE
...
def main():
    ...
    global snake_x, snake_y, direction, score, key_, paused, highscore
    global game_ended, food_x, food_y, username, username_list, highscore_list
    snake_x = [400, 380, 360]
    snake_y = [400, 400, 400]
    ...
    def create_objects():
        ...
        for z in range(len(snake_x)):
            c.create_image(snake_x[z], snake_y[z],
                           image=snake_img, tag="snake_block")
        # TEXT:
        c.create_text(50, 15, text="Score: " + str(score),
                      fill="#fff", tag="score")
        c.create_text(100, 15, text="|", fill="#fff")
        c.create_text(167, 15, text="highscore: " +
                      str(highscore), fill="#fff", tag="highscore")
        c.create_text(1170, 15, text="Press p/b to pause game.", fill="#fff")

        i = 0
        for m in range(22):
            # VERTICAL LINES:
            c.create_line(30 + i, 30, 30 + i, 710, fill="#476042")
            c.create_line(50 + i, 30, 50 + i, 710, fill="#476042")
            c.create_line(10 + i, 30, 10 + i, 710, fill="#476042")
            # HORIZONTAL LINES:
    ...
    def direction_and_keys():
        ...
        if(key_ == "p"):
            toggle_pause()
            quit_btn = Button(text="Quit and save game", height=5,
                              width=18, command=save_and_exit)
        ...
        if(key_ == "c"):
            ...
            c.create_image(snake_x[-1], snake_y[-1],
                           image=snake_img, tag="snake_block")
            score += 1
            ...
            if (score > highscore):
                highscore = score
                c.itemconfigure("highscore", text="highScore: " +
                                str(highscore), fill="#fff")
        # MOVE_SNAKE:
        if(direction == "Right"):
            new_x = snake_x[0] + 20
            new_y = snake_y[0]
    ...
    def check_snake_collision():
        ...
        if(snake_x[0] == 1280 or snake_x[0] == 0):
            game_ended = True
            game_over()
        if(snake_y[0] == 720 or snake_y[0] <= 20):
            game_ended = True
            game_over()
    ...
    def update_key(key):
        ...
        opp_directions = [["Up", "Down"], ["Left", "Right"],
                          ["Down", "Up"], ["Right", "Left"]]
        if(temp_direction in directions and [temp_direction, direction]
           not in opp_directions):
            direction = temp_direction
        if(temp_direction == "p" or temp_direction == "b" or
           temp_direction == "c"):
            key_ = temp_direction
    ...
    def game_over():
        ...
        for z in range(len(username_list)):
            username_highscore_list.append([username_list[z],
                                           highscore_list[z]])
        ...
        for i in range(len(username_list)):
            ...
            c.create_text(640, 100 + m, text=username_highscore_list[i][0],
                          fill="#fff")
            c.create_text(840, 100 + m, text=str(username_highscore_list
                          [i][1]), fill="#fff")
            m += 50
    ...
    def loop_functions():
        ...
        if(game_ended is True):
            retry_btn = Button(text="Retry ", height=5, width=20,
                               command=lambda: [del_canvas(), main()])
            retry_btn.place(x=5, y=635)
...
start = Button(window, text="Start Game ", height=5, width=18,
               highlightbackground='yellow',
               command=lambda: [check_if_load_game(), main()])
quit = Button(window, text="Quit Game", height=5, width=18,
              highlightbackground="red", command=quit)
